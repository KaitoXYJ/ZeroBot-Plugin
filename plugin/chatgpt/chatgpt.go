...
var mainlimit = ctxext.NewLimiterManager(time.Second*3, 1)
...
func init() {
	engine := control.Register("chatgpt", &ctrl.Options[*zero.Ctx]{
		...
		Help: "@bot[对话内容]\n" +
			"#添加预设(x) xxx(需要SU权限)\n" +
			"#设置预设(x)(需要SU权限)\n" +
			"(私聊发送)设置OpenAI apikey [apikey](需要SU权限)\n" +
			"反并发-同时回复多条问题会报错\n" +
			"10秒内只会回应1次\n",
	...
	engine.OnRegex(`([\s\S]*)`, zero.OnlyToMe).Limit(mainlimit.LimitByGroup).SetBlock(true).
	...
	engine.OnRegex(`^#设置预设(\d)$`, zero.SuperUserPermission).SetBlock(true).
		Handle(func(ctx *zero.Ctx) {
			num := ctx.State["regex_matched"].([]string)[1]
	...
	engine.OnRegex(`^#删除预设$`, zero.SuperUserPermission).SetBlock(true).
		Handle(func(ctx *zero.Ctx) {
			group := strconv.Itoa(int(ctx.Event.GroupID))
